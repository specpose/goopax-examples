...
bool test_dev(unsigned int env)
{
    ...
    // throw std::runtime_error("Default Device in ENV "+std::to_string(env)+" ("+backend_names[log_env]+")"+" not
    // valid.");
    return false;
}

/*  Logic   */
std::array<Py_ssize_t, PyBUF_MAX_NDIM> make_strides(const std::array<Py_ssize_t, PyBUF_MAX_NDIM>& shape,
                                                    const Py_ssize_t itemsize)
{
