...
py::object pynumpy_test_ndarray()
{
    ...
    if (PyObject_GetBuffer(buffy, view, PyBUF_C_CONTIGUOUS | PyBUF_FORMAT) == 0)
    {
        ...
        np::ndarray tmp = np::from_data(view->buf,
                                        dtype,
                                        py::make_tuple(sizeof(test_data) / sizeof(T)),
...
static PyMethodDef pylist_methods[] = { { "process_list", pylist_process_list, METH_VARARGS, "" },
...
static struct PyModuleDef pylist_module = {
    PyModuleDef_HEAD_INIT, "pylist", "", 0, pylist_methods, NULL, NULL, NULL, NULL
};
